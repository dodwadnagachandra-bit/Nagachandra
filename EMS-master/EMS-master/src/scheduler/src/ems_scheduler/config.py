"""Schedule config loader with JSON Schema validation.

Loads schedule_config.yaml and validates it against the JSON Schema at
config/schemas/schedule_config.schema.json. Raises ValueError with a
...
# Default path to the JSON Schema for schedule_config
...
def load_schedule_config(
    ...
    """Load and validate a schedule_config YAML file.

    Reads the YAML at ``path``, validates it against the JSON Schema at
    ``schema_path`` (defaults to config/schemas/schedule_config.schema.json),
    and returns the validated configuration as a plain dict.

    Args:
        path: Path to the schedule_config.yaml file to load.
        schema_path: Path to the JSON Schema file. Defaults to
            config/schemas/schedule_config.schema.json relative to CWD.
    ...
    if not resolved_schema_path.exists():
        msg: str = f"Schedule config schema not found: {resolved_schema_path}"
    ...
    # Load config YAML
    if not path.exists():
        msg = f"Schedule config file not found: {path}"
        raise FileNotFoundError(msg)
    ...
    if not isinstance(config, dict):
        msg = f"Schedule config must be a YAML mapping, got {type(config).__name__}"
    ...
    if errors:
        ...
        msg = (
            f"schedule_config validation failed at '{field_path}': {first.message}"

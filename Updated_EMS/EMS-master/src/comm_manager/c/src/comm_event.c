...
/**
 * @file comm_event.c
 * @brief mpack MessagePack encoding + non-blocking ZMQ send for comm events.
 *
 * Follows the same pattern as safety_event.c. All encoding uses a
 * stack-allocated 512-byte buffer. No heap allocation in the hot path.
 * ZMQ sends use ZMQ_DONTWAIT and silently drop on EAGAIN.
 *
...
/** Stack buffer size for MessagePack encoding. */
#define EVENT_BUF_SIZE 512

/** ZMQ send high-water mark for comm event sockets. */
                    ...
                    const char *push_addr)
{
    ...
    if (!ctx->zmq_ctx)
    {
        fprintf(stderr, "[comm_event] ERROR: zmq_ctx_new failed\n");
        return -1;
    }
    ...
    if (!ctx->pub_sock)
    {
        fprintf(stderr, "[comm_event] ERROR: zmq_socket(PUB) failed\n");
        goto fail_ctx;
    }

    int hwm = EVENT_HWM;
    int linger = 0;  /* Don't block on zmq_close */
    ...
    {
        fprintf(stderr, "[comm_event] ERROR: zmq_connect(PUB, %s) failed: %s\n",
    ...
    if (!ctx->push_sock)
    {
        fprintf(stderr, "[comm_event] ERROR: zmq_socket(PUSH) failed\n");
        goto fail_pub;
    }
    ...
    {
        fprintf(stderr, "[comm_event] ERROR: zmq_connect(PUSH, %s) failed: %s\n",
                push_addr, zmq_strerror(zmq_errno()));
        goto fail_push;
    }

    ctx->seq = 0;
    fprintf(stderr, "[comm_event] initialized (PUB=%s, PUSH=%s)\n",
            pub_addr, push_addr);
    return 0;
...
/**
 * Encode a comm event into MessagePack envelope.
 *
                           ...
                           const char *message, ems_severity_t severity)
{
    ...
    mpack_write_cstr(&writer, "comm_manager_c");

    /* topic */
    mpack_write_cstr(&writer, EMS_MSG_KEY_TOPIC);
    mpack_write_cstr(&writer, EMS_TOPIC_COMM_FAULT);
    ...
    {
        fprintf(stderr, "[comm_event] ERROR: mpack encoding failed\n");
        return 0;
    }
...
/**
 * Encode a comm event with extended device data.
 *
 * Payload map has 5 keys: event_type, message, severity, device_id, device_address
 *
 * @return Number of bytes written, or 0 on error.
 */
static size_t encode_event_with_data(char *buf, size_t buf_size,
                                     uint64_t seq, const char *event_type,
                                     const char *message, ems_severity_t severity,
                                     const char *device_id, int device_address)
{
    ...
    /* payload -- inner map with event details + device data */
    mpack_write_cstr(&writer, EMS_MSG_KEY_PAYLOAD);
    mpack_start_map(&writer, 5);
    ...
    mpack_write_cstr(&writer, "device_id");
    mpack_write_cstr(&writer, device_id);

    mpack_write_cstr(&writer, "device_address");
    mpack_write_int(&writer, device_address);
    ...
    if (mpack_writer_destroy(&writer) != mpack_ok)
    {
        fprintf(stderr, "[comm_event] ERROR: mpack encoding (ext) failed\n");
        return 0;
    }
...
/**
 * Send an encoded message on PUB and PUSH sockets.
 *
 * PUB: 2 frames [topic | SNDMORE] [msgpack body]
 * PUSH: single frame [msgpack body]
 * Matches Python convention -- no length prefix frame.
 */
static void send_event(comm_event_ctx_t *ctx, const char *buf, size_t msg_len)
{
    /* PUB socket: 2 frames [topic | SNDMORE] [msgpack body] */
    if (ctx->pub_sock)
    {
        const char *topic = EMS_TOPIC_COMM_FAULT;
    ...
    /* PUSH socket: single frame [msgpack body] */
    if (ctx->push_sock)
    {
                                 ...
                                 const char *device_id, int device_address)
{
    ...
    size_t msg_len = encode_event_with_data(buf, sizeof(buf), seq, event_type,
...
void comm_event_close(comm_event_ctx_t *ctx)
{
    ...
    fprintf(stderr, "[comm_event] closed\n");
}
